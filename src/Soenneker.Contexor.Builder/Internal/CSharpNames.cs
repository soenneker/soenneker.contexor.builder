...
internal static class CSharpNames
{
    ...
    {
        name.ThrowIfNullOrWhiteSpace(parameter);
    ...
    internal static string Identifier(string value)
    {
        ...
        {
            if (!c.IsAsciiLetterOrDigit()) { upper = true; continue; }
            if (result.Length == 0 && c.IsAsciiDigit()) result.Append('_');
            result.Append(upper ? c.ToUpperInvariant() : c);
            upper = false;
        }
