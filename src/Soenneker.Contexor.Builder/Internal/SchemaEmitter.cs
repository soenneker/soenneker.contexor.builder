...
internal sealed class SchemaEmitter
{
    ...
    private readonly Uri _baseUri = new("https://contexor.invalid/schema");
    ...
    internal SchemaEmitter(JsonObject document, ContexorBuilderOptions options, CancellationToken ct)
    {
        ...
        if (options.ClientName is "OptionalConverter" or "JsonTypeCache" or "Optional" or "OptionalConverterFactory" or "Models" ||
            options.ClientName.StartsWith("IOptional", StringComparison.Ordinal))
            throw new ArgumentException("The client name conflicts with generated infrastructure.",
                nameof(options.ClientName));
        _names.UnionWith([
            options.ClientName, "I" + options.ClientName, "Optional", "OptionalConverterFactory", "Models"
        ]);
        _names.UnionWith([
    ...
    internal ContexorBuildResult Finish()
    {
        ...
        return new ContexorBuildResult(new ReadOnlyDictionary<string, string>(_files),
            "global::" + _options.Namespace + "." + _options.ClientName, _diagnostics.AsReadOnly());
    }
    ...
    private void EmitSerialization()
    {
        ...
        using var registration = new PooledStringBuilder(4096);
        ...
        foreach (string type in new[] { "OptionalConverterFactory", "OptionalConverter", "JsonTypes", "JsonTypeCache", "PooledJsonBuffer" })
        ...
        types.Add(_element);
        using var attributes = new PooledStringBuilder(16384);
        ...
        _files["RpcJsonContext.cs"] = Template("RpcJsonContext").Replace("@@NAMESPACE@@", _options.Namespace)
    ...
    {
        using Stream stream =
            typeof(SchemaEmitter).Assembly.GetManifestResourceStream("Contexor.Templates." + name + ".txt") ??
            throw new InvalidOperationException("Missing generation template: " + name);
        return stream.ToStrSync(leaveOpen: true);
    }
    ...
    private void Index(JsonNode schema, Uri scope)
    {
        ...
        {
            if (!ReferenceEquals(schema, _root) && obj["$schema"] is JsonValue dialect && dialect.TryGetValue(out string? dialectText) &&
                dialectText is not ("http://json-schema.org/draft-07/schema#"
                    or "http://json-schema.org/draft-07/schema"))
                throw new NotSupportedException("Only JSON Schema draft 7 is supported; found " + dialectText);
    ...
    private JsonNode Resolve(JsonNode schema, string reference)
    {
        ...
        string fragment = target.Fragment.ToUnescaped();
    ...
    private string BuildType(JsonNode schema, string suggestion)
    {
        ...
        if (types.Count > 1)
            return Fallback(schema, "Multiple non-null types");
        ...
        string result = kind switch
        {
            ...
            "number" => "decimal",
            "boolean" => "bool",
            _ when nullable || obj.Count == 0 || obj.All(p => p.Key is "description" or "title" or "default" or "$schema") => _element,
    ...
    private string ReferenceName(JsonNode target, string reference, string fallback)
    {
        ...
        return tail is "#" or "" ? fallback : tail.ToUnescaped().Replace("~1", "/").Replace("~0", "~");
    }
    ...
    private string EmitUnion(JsonNode schema, string suggestion, JsonArray union, bool exclusive)
    {
        ...
        using var code = new PooledStringBuilder(4096);
    ...
    private string EmitObject(JsonNode schema, string suggestion, JsonObject obj, List<JsonObject>? parts = null)
    {
        ...
        {
            JsonNode property = candidates[0];
            string member = CSharpNames.Unique(jsonName, memberNames);
            string type = candidates.All(x => JsonNode.DeepEquals(property, x))
